...
<html>
	<head>
		...
		function sendMasterJoin(f) {
			let a_id = f.a_id.value.trim(); //아이디
			...
			let email_check = f.email_check.value.trim(); //이름
			...
			if(tel != ''){
			 ...
			 //-----------------전화번호 입력하지 않았을 때
	        if(tel == ''){
	            if(!confirm("전화번호 정보를 입력하지 않으셨습니다.\n회원가입을 계속 진행하시겠습니까?")){
					return;
	            } 
	        } 
	        
        //------------------------Ajax로 회원가입 성공 띄우기
	        let url = "join_admin_insert.do";
      ...
      //----------------------------회원가입 Ajax joinResult 함수 호출
        function joinResult() {
			if(xhr.readyState == 4 && xhr.status == 200){
				...
				if(json[0].result == 'success'){
					alert(json[0].name + " 관리자님 회원가입을 환영합니다");
					location.href = "admin_login_main.do";
					//관리자/회원용 로그인 창으로 이동해야 함.
					return;
				}
			}
		}
        ...
        //아이디 중복확인 함수 호출
        function duplicate_checkID(a_id) {
        	 let idError = document.getElementById("id-error");
             let inputID = f.a_id.value;
          ...
          // 대문자가 포함되어 있는지 확인
             ...
             let url = "duplicate_check_adminID.do";
             let param = "a_id=" + inputID;
             sendRequest(url, param, check_adminID, "post");
		}//---------------------duplicate_checkID
		...
		function resetAndCheckID() {
            ...
            let idError = document.getElementById("id-error");
            idError.textContent = "";
            let a_id = document.getElementsByName("a_id")[0].value;
           
            if (a_id == '' || a_id != null) {
                return; // 아이디가 비어있으면 체크하지 않음
            }
            // 중복 체크 함수 호출
            duplicate_checkID(a_id);
		}
		...
		<div class="box">
            <div class="text-center">
                <div class="id-input input-group">
                    <input autofocus class="id" name="a_id" placeholder="* 아이디" id="a_id" oninput="resetAndCheckID()">
                    <input type="button" value="중복 확인" class="check_id" onclick="duplicate_checkID(a_id)"></input>
                </div>
            ...
            <div class="row text-center input-group">
                <input value="가입하기" class="join_btn" type="button" onclick="sendMasterJoin(this.form)"></input>
            </div>
        </div>
	</form>		
</body>
