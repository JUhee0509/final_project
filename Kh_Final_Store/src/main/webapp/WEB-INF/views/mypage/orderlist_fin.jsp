...
<html>
	<head>
		...
		<script>
			...
			function exchange(o_idx) {
				if(!confirm("교환 하시겠습니까?")){
					return;
				}
				location.href = "exchange.do?o_idx="+o_idx;
			}
			
			function refund_update(o_idx) {
				if(!confirm("환불 하시겠습니까?")){
					return;
				}
				location.href = "refund_update.do?o_idx="+o_idx;
			}
		</script>
	...
	<body>
		...
		<h2 class="fin" align="center">주문 내역</h2>
		<br>
	...
	<c:forEach var="vo" items="${list}">
		...
		<div class="row">
			<div class="col" id="a1">
				<input type="button" class="bttn" value="교환" onclick="exchange(${vo.o_idx})">
				<input type="button" class="bttn" value="환불" onclick="refund_update(${vo.o_idx})">
			</div>
		    ...
		    <div class="col" id="a2">
		      <a href="review_write.do?p_idx=${vo.p_idx }">리뷰작성</a>
		    </div>
		</div>
		<hr>
	</c:forEach>	
