...
<head>
    ...
    <script>
        function findID(f) {
            ...
            let url = "storeUserFindId.do";
            let param = "name=" + name + "&email=" + encodeURIComponent(email);
            sendRequest(url, param, userFindID, "post");
        }
        ...
        function userFindID() {
            if(xhr.readyState == 4 && xhr.status == 200){
                ...
                if(json[0].result == 'email_no'){
                	...
                	location.href = "show_Name_ID.do?email="+json[0].email;
                }
            }
        }
        
        function closeModal() {
            document.getElementById('modal_email').style.display = 'none';
            document.getElementById('modal_name').style.display = 'none';
            document.getElementById('modal_total').style.display = 'none';
        }
    </script>
     <style>
        body {
...
<body>
    <div class="fixed-top">
	    ...
	    <div class="box">
	    	...
	    	<div class="find_PWD">
                <a href="storeUserFindPWD.do">비밀번호를 잊어버리셨나요?</a>
            </div>
        </form>
